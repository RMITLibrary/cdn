...
function ParseElements(data, elmID) {
  try {
    ...
    var JSONdata = data; // Get the container that will hold our table

    var theContainer = document.getElementById(elmID); // Clear the placeholder non-JS content

    theContainer.innerHTML = ""; // Create an <ol> to contain it all and add it to the page

    var ol = document.createElement("ol");
    ol.setAttribute("id", "Table");
    theContainer.appendChild(ol); // Grab the JSON nodes

    var filteredNodes = JSONdata.elements; // Blank category array

    var catLookup = {};
    var catArray = []; // Loop through the elements nodes

    for (var a = 0; a < filteredNodes.length; a++) {
      var json = filteredNodes[a]; // Create the <li>, give it an id and row/col info

      var elementNode = document.createElement("li");
      elementNode.setAttribute("id", json.symbol);
      elementNode.classList.add("row" + json.ypos);
      elementNode.classList.add("col" + json.xpos); // Get the category and concatenate into the class

      var rawCat = json.category;
      var stringCat = rawCat;
      stringCat = stringCat.split(' ').join('_');
      stringCat = stringCat.split(',').join('');
      elementNode.classList.add("cat-" + stringCat); // Make the atomic number container

      var numberDiv = document.createElement("div");
      numberDiv.innerHTML = json.number; // Make the name container and give it an id

      var nameDiv = document.createElement("div");
      nameDiv.setAttribute("id", "name" + json.symbol);
      nameDiv.innerHTML = json.name; // Make tye symbol container

      var symbolDiv = document.createElement("div");
      symbolDiv.innerHTML = json.symbol; // Make the atomic mass container

      var massDiv = document.createElement("div");
      massDiv.innerHTML = json.atomic_mass; // Make the button and give it an id and ARIA bits
      ...
      detailButton.setAttribute("onclick", "OpenDialog(this.id,'" + json.name + "');"); // Add all the nodes to the <li>
      ...
      elementNode.appendChild(detailButton); // Add the <li> to the <ol>

      ol.appendChild(elementNode); // Loop through the category and add distinct to array

      if (!(rawCat in catLookup)) {
        catLookup[rawCat] = 1;
        catArray.push(rawCat); // console.log(rawCat);
      }
    ...
    for (var i = 0; i < catArray.length; i++) {
      ...
      catString = "cat-" + catString; // console.log(catString);
      // Make the color container and class it

      var dt = document.createElement("dt");
      dt.classList.add(catString); // Make the category container

      var dd = document.createElement("dd");
      dd.innerHTML = catText;
      dd.setAttribute("id", catString); // Make the button
      ...
      showButton.setAttribute("aria-labelledby", labelledby); // Append these to the <dl>
    ...
    keyItem.setAttribute("role", "presentation"); // keyItem.setAttribute("aria-hidden","true");

    keyItem.setAttribute("id", "Key"); // Give it some context

    var h2 = document.createElement("h2");
    h2.innerHTML = "Element Categories:"; // Add the context text

    keyItem.appendChild(h2); // Add the new list to it

    keyItem.appendChild(dl); // Add this entire mess to the <ol>
...
function ToggleStyleBlock(strClass, showhide) {
  try {
    if (showhide == "show") {
      ...
      styleBlock.sheet.insertRule("#Elements li:not(#Key):not(." + strClass + ") { background-color: #999; opacity: .5; }", 0);
      styleBlock.sheet.insertRule("@media screen and (prefers-color-scheme: dark) { #Elements li:not(#Key):not(." + strClass + ") { background-color: #333; opacity: .5; } }", 1);
      styleBlock.sheet.insertRule("@media screen and (-ms-high-contrast: active) { #Elements li:not(#Key):not(." + strClass + ") { opacity: .25; } }", 2);
    } else {
...
function OpenDialog(eID, elName) {
  try {
    DialogTrigger = eID; // Get all the elements to manipulate
    ...
    var closeBtn = document.getElementById("DialogClose"); // Set the heading

    heading.innerHTML = elName; // Call the function to populate the dialog

    ParseElementDetail(elements, "ElementDetail", elName); // Hide the content regions from AT
    ...
    overlay.style.display = "block"; // Add click handler to overlay

    overlay.setAttribute("onclick", "CloseDialog('" + eID + "');"); // Kill the page scroll

    body[0].style.overflow = "hidden"; // Set the dialog to modal

    dialog.setAttribute("aria-modal", "true"); // dialog.setAttribute("data-id",eID);

    dialog.removeAttribute("hidden"); // Put focus on the close button
...
function CloseDialog(eID) {
  try {
    ...
    var overlay = document.getElementById("Overlay");
    var dialog = document.getElementById("Dialog");
    var triggerBtn = document.getElementById(eID); // Make the regions available to AT
    ...
    overlay.style.display = "none"; // Return the scrollbar

    body[0].style.overflow = "auto"; // Kill the dialog

    dialog.removeAttribute("aria-modal");
    dialog.removeAttribute("data-id");
    dialog.setAttribute("hidden", ""); // Return focus to trigger
...
function ParseElementDetail(data, elmID, elName) {
  try {
    ...
    theContainer.innerHTML = ""; // Use this if the JSON is coming over the wire
    // var JSONdata = JSON.parse(data);

    var JSONdata = data; // Grab the JSON nodes

    var filteredNodes = JSONdata.elements; // Loop through the elements nodes
    ...
    for (var a = 0; a < filteredNodes.length; a++) {
      ...
      if (json.name == elName) {
        ...
        theContainer.appendChild(p); // Create a <dl> to hold categories

        var dl = document.createElement("table");
        dl.className = 'table table-striped';
        var tbody = document.createElement("tbody");
        dl.appendChild(tbody); // Symbol
        ...
        if (json.number != undefined) {
          var tr = document.createElement("tr");
          var dt = document.createElement("th");
          dt.innerHTML = "Atomic Number";
          var dd = document.createElement("td");
          dd.innerHTML = json.number;
          tbody.appendChild(tr);
          tr.appendChild(dt);
          tr.appendChild(dd);
        } // Atomic mass
        ...
        if (json.shells != undefined) {
          var tr = document.createElement("tr");
          var dt = document.createElement("th");
          dt.innerHTML = "Electron shell configuration";
          var dd = document.createElement("td");
          dd.innerHTML = json.shells;
          tbody.appendChild(tr);
          tr.appendChild(dt);
          tr.appendChild(dd);
        }

        theContainer.appendChild(dl); // Source
        ...
        if (json.source != undefined) {
          ...
          a.innerHTML = "Source, more information: Wikipedia";
          p.appendChild(a);
